...
Use readAnchorSection(...) for one H2 section or nested path (for example \`Current State > Capabilities\`) advertised by an anchor's availableSections/availableSectionPaths. Use readAnchor(...) only when you need the full body.
...
the index when your workflow checks in that file.`,
  ...
  server.registerTool(
    "readAnchorSection",
    {
      ...
      description:
        "Read one H2 section or nested heading path without loading the entire anchor. Pass an H2 name such as `Current State` or a path such as `Current State > Capabilities`; use paths advertised in loadContext/startTask availableSections and availableSectionPaths. " +
  ...
  server.registerTool(
    "startTask",
    {
      ...
      description:
        ...
        "Project context anchors load front matter plus the complete Introduction-through-Invariants design header and advertise remaining H2 and nested heading paths for readAnchorSection. " +
  ...
  server.registerTool(
    "loadContext",
    {
      ...
      description:
        ...
        "Loaded anchors include canonical sectionDefinitions guidance. " +
        "In excerpt mode, project context anchors return their complete Introduction-through-Invariants design header plus availableSections and availableSectionPaths for on-demand reads. " +
