...
export type RoutingDiagnostics = {
  ...
  /** Records served and never used. A scope full of these is dead weight in every bundle it appears in. */
  neverUsed: Array<{ routeKey: string; offered: number; used: number }>;
...
export async function routingDiagnostics(
  ...
  const neverUsed = await pool.query<{ route_key: string; offered: string; used: string }>(
    `SELECT i.route_key,
            count(DISTINCT i.impression_guid) AS offered,
