...
export async function setRecordScopes(input: SetRecordScopesInput): Promise<SetRecordScopesResult> {
  ...
  const command = await input.handler.execute({
    ...
    //
    // Both inputs are hashed rather than embedded, matching the importer's retirement key. A
    // stable key is a document path plus a heading path and a scope set is unbounded, so the
    // literal form can outgrow the btree entry limit on `commands.idempotency_key` — which
    // would fail at write time, on the size of the caller's data rather than anything it did
    // wrong.
    idempotencyKey:
      input.idempotencyKey ??
      `record.setScopes:${input.recordType}:${createHash("sha256")
        .update(
          // NUL as the separator, written as an escape rather than a literal byte. It is
          // the right separator -- it cannot occur in a slug or a stable key, so no two
          // different inputs can collide by rearranging the boundary -- but typed raw it
          // makes Git classify this file as binary, and a binary file has no reviewable
          // diff. Not cosmetic: the automated review of PR #121 silently covered 3 of 4
          // files because it could not read this one. The escape produces the identical
          // string, so every idempotency key derived so far still matches.
          `${input.stableKey ?? input.recordGuid}\u0000${desired.join("\u0000")}`,
        )
        .digest("hex")
        .slice(0, 32)}`,
    reason: input.reason,
    ...
    apply: async (tx) => {
      ...
      for (const slug of retired) {
        ...
        await tx.query(
          // `retired_by_correction` is what keeps this from being undone by the next import.
          // Import retires associations too, when their document leaves the pinned commit, and
          // reinstates them when it returns; the mark is the only thing distinguishing "the file
          // went away" from "a person decided this was wrong".
          `UPDATE "${schema}".record_scopes SET retired_at = now(), retired_by_correction = true
      ...
      for (const slug of added) {
        // Adding a scope back withdraws the earlier decision to remove it. The mark records what
        // the human currently wants, not everything they have ever wanted, so it is cleared here
        // — otherwise a change of mind would leave import permanently forbidden from deriving
        // this association again, and the suppression would outlive the intent behind it.
        await tx.query(
          `UPDATE "${schema}".record_scopes SET retired_by_correction = false
            WHERE workspace_guid = $1 AND scope_guid = $2 AND retired_by_correction
              AND ($3::text IS NULL OR stable_key = $3)
              AND ($4::uuid IS NULL OR record_guid = $4)`,
          [
            input.workspaceGuid,
            scopes.get(slug)!,
            input.stableKey ?? null,
            input.stableKey ? null : owner.recordGuid,
          ],
        );
        await tx.query(
