...
`,
  ...
  it("builds a grouped dynamic context root and excludes archive by default", async () => {
    ...
    expect(root.entries.map((entry) => entry.category)).toEqual(["server-rules", "agent-rules", "projects", "shared"]);
  ...
  it("loadContext returns entries plus excerpt anchors in one call", async () => {
    ...
    expect(loaded.totalMatching).toBe(2);
    expect(loaded.returnedCount).toBe(2);
